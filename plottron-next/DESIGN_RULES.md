...
- 画像・動画・SVG は `plottron-next/public/` のみ使用（プロジェクト直下 `images/` は非推奨）
- 用途別ディレクトリ化（例：`public/hero/`, `public/icons/`）
- 画像は最適化必須（写真: webp/jpg、透過: png、ベクタ: svg テキスト）

## 17. Vercel 設定ルール

- Root Directory はダッシュボードで `plottron-next` を指定
- `vercel.json` は原則作成しない（必要時のみ最小設定）
- Build: `npm run build` / Output: `.next`（自動検出）
- 環境変数はダッシュボード管理（`.env` は `.gitignore`）

## 18. Next.js / 構成ルール

- App Router（`app/`）のみ使用、`pages/` は追加しない
- `next.config.ts` は保守最小限（実験フラグ禁止、出力方式を頻繁に変更しない）
- ルート直下に `index.html` は置かない（Next が生成/配信）

## 19. 型 / リンティング運用

- PR で `npm run lint` と型検査（`tsc --noEmit` または `next build`）を必須
- `any` は原則禁止。やむを得ない場合はファイル先頭に抑制コメントと TODO で除却計画を明記
- 共通型は `types/` に追加し再利用

## 20. スタイル/デザイン保全の強化

- 既存トークン外の任意値使用は「例外運用」で申請
- `globals.css` への追記は原則禁止。まずはコンポーネント内で閉じる案を検討
- `prefers-reduced-motion` を尊重し、負荷を抑制

## 21. パフォーマンス予算（目安）

- LCP < 2.5s / CLS < 0.1 を目標
- 初回ロード JS（圧縮後）≦ 150KB を目安に依存追加を精査
- 画像は表示解像度の 1.5 倍以内、不要な prefetch/preload は避ける

## 22. アクセシビリティの強化

- `role` / `aria-*` を適切に設定
- フォーカスリングは可視性を維持しカスタムで隠さない
- PR で AA コントラストを自動/目視で確認（スクショ添付）
