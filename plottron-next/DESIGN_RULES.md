...
- 原本：リポジトリ直下 `original-assets/` に保管（最適化前）。
- 配布：最適化後を `plottron-next/public/` 配下で用途別ディレクトリへ（例：`public/hero/`, `public/logos/`）。
...
### 9.1 PC 専用調整の原則（モバイルは不変）

- 指示例「PC の画像を大きく」は、`lg` 以上のブレークポイントでのみ適用すること（`sm`/`md`は変えない）
- 実装例（Tailwind）：
...
- 確認：`sm/md/lg` の 3 段階で比較スクショを PR に添付
...
### 12.1 メタデータ管理
- **基本メタ**: `app/layout.tsx` で `metadata` オブジェクトを設定
- **ページ別メタ**: 各ページで `generateMetadata` 関数を使用
- **必須項目**: title, description, keywords, og:title, og:description, og:image
- **構造化データ**: JSON-LD で作品情報・サイト情報を記述

### 12.2 画像最適化
- **alt属性**: 意味のある代替テキストを必ず設定
- **ファイル名**: 検索エンジンが理解しやすい命名（例: `plottron-vol1-cover.jpg`）
- **サイズ最適化**: WebP形式優先、適切な解像度設定
- **遅延読み込み**: `loading="lazy"` でパフォーマンス向上

### 12.3 URL構造
- **セマンティック**: `/comics/[id]`, `/genres/[genre]` など意味のあるURL
- **短縮**: 3階層以内、不要なパラメータを避ける
- **一貫性**: スラッシュの有無・大文字小文字を統一

### 12.4 コンテンツ最適化
- **見出し構造**: h1 → h2 → h3 の階層を適切に設定
- **キーワード配置**: title, h1, 最初の段落に重要キーワードを配置
- **内部リンク**: 関連作品・ジャンル間のリンクを充実
- **更新頻度**: 定期的なコンテンツ更新でクロール頻度向上

### 12.5 技術的SEO
- **サイトマップ**: `/sitemap.xml` で全ページを明示
- **robots.txt**: クロール制御・検索エンジン向け指示
- **ページ速度**: Core Web Vitals（LCP, FID, CLS）の最適化
...
export const metadata: Metadata = {
  ...
  keywords: ['comic', 'digital', 'interactive', 'PLOTTRON', 'manga'],
  openGraph: {
    title: 'PLOTTRON - Digital Comic Accelerator',
    description: 'Interactive digital comic experience',
    images: ['/og-image.jpg'],
    type: 'website'
  },
  twitter: {
...
- [ ] 画像の原本は `original-assets/`、最適化版は `public/` に配置
...
## 21. 拡張運用ガイド（カラー/カテゴリ/ページ/ロゴ/サイズ）
...
## 22. CSS/クラス命名と重複防止（重要）
- 原則：Tailwind を優先。カスタム CSS クラスは最小限・スコープ限定（コンポーネント単位）
- 命名：`<Component>-<role>` または BEM 風（`Component__element--modifier`）。接頭辞で衝突回避（例：`pl-`）
- 追加前チェック：同名クラス/同機能クラスが無いか検索（`class="..."` / `styles/` / `components/` を横断）
- 重複禁止：同等の見た目・機能のクラス/コンポーネントを増やさない。既存を拡張 or 共通化
- 共有化：共通ロジックはユーティリティ/フック/ベースコンポーネントへ抽出
